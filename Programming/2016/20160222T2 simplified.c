...
void enqueue (node_t **, node_t *);//function to change the linear list into circular list
  ...
  node_t **p2)
{
 ...
 // if (pTail1->next!=NULL) { //pTail1 points to the last structure and pTail->next actually points to the first one of the list
...
//insert to the new lists implementations
void
enqueue (
  node_t **head,
  node_t *p
)
{
  ...
  if (*head==NULL) { //only run one time, the first time
    *head = p;
    p->next=NULL;
 } else {
